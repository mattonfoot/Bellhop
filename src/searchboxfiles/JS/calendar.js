...
var Calendar = new Class({	
  Implements: Options,
	options: {
		...
		tweak: {x: 2, y: 0} // tweak calendar positioning
	},
	...
	initialize: function(obj, options) {
		...
		for (var i in obj) {
			var cal = { 
				button: new Element('div', { 'id': 'calicon' }),
				el: $(i),
				els: [],
				id: id++,
				month: d.getMonth(),
			...
			if (el.get('tag') == 'select') {		
				if (el.format.test('(y|Y)')) { // search for a year select
					...
					if (years[0] > start.getFullYear()) { 
						var d = new Date(years[0], start.getMonth() + 1, 0); // last day of new month
	...
	changed: function(cal) {
		...
		var selectedIndex = cal.el.selectedIndex;
		
		cal.els.each(function(el) {	// then we can set the value to the field
			el.value = this.format(cal.val, el.format); 		
		}, this);
		
		//if the use selects today's date then it should show today's date rather than the text 'Arrival Date'
		if(selectedIndex == 1 && cal.el.selectedIndex == 0 ) 
		    (cal.el.selectedIndex  = 1)
...
window.addEvent('domready', function() { 
    searchCalendar = new Calendar({date: 'Ymd'});
});
